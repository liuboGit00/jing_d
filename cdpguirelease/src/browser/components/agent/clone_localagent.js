...
class Localagent extends Component {
        ...
        if(items.length>0 && download!=undefined){
            for(let i=0;i<items.length;i++){
                if(download[0] !=undefined){
                    ...
                    if(items[i].id == id && (download[0].task.split('.').pop()).split('l')[0] == 'c'&& (download[0].task.split('.').pop()).split('clone')[1] == '2agentjob'){
...
export default createForm({
 ...
 /*<div>
                <Modal ref="modal"
                    width = {350}
                    visible={this.props.visible}
                    title="数据传输" onOk={this.props.onOk} onCancel={this.props.onCancel}
                    footer={[
                    ...
                    >
                    <table width="300"  height="150"    className="agentclone_table">
                        <tbody>
                            <tr>
                                <td>块大小:</td>
                                <td >
                                    <select ref="bs" className="agentclone">
                                        <option key='512' value="512">512</option>
                                        <option key='1024' value="1024">1024</option>
                                        <option key='2048' value="2048">2048</option>
                                        <option key='4096' value="4096">4096</option>
                            ...
                            <tr>
                                <td>传输起始位置:</td>
                                <td >
                                   <input ref="offset" className="agentclone_input" placeholder=""/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
