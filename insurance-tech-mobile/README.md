...
样式统一在 style 文件夹里维护
    ...
    |- common.less  里面放公共的、全局的、使用频率比较高的样式，这样可以方便组织样式。如：.hide{display: none;}
    |- mixin.less   里面放全局的样式方法，已经在webpack中全局引入，使用的时候 @include ct();
    |- reset.less   里面放页面样式重置的css，不需要修改
    |- var.less     里面放公共变量，方便全局样式，如：颜色、字体大小等
...
方法一：
在scoped的less中使用 >>> 或者 /deep/ ，它们是vue提供的"深度作用选择器"，你可以参考 —— https://vue-loader.vuejs.org/zh/guide/scoped-css.html#%E6%B7%B1%E5%BA%A6%E4%BD%9C%E7%94%A8%E9%80%89%E6%8B%A9%E5%99%A8
<style scoped>
    .a >>> .b { /* ... */ }
    .a /deep/ .b { /* ... */ }
</style>
...
同时你也可以传入一些配置项来设置border的颜色、粗细等等，详细的请切换到 mobile_template 分支，查看 src/style/mixin.less 里面的 border 类。
```
